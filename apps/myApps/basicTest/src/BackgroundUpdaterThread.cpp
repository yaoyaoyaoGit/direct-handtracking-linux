...
void BackgroundUpdaterThread::threadedFunction() {
	...
	while(isThreadRunning()) {
		...
		uint32_t *debugpx = (uint32_t *)backgroundStateDebug.getPixels().getData();
		float *means = bgmean.getData();
		float *stdevs = bgstdev.getData();
...
BackgroundUpdaterThread::BackgroundUpdaterThread(ofxFreenect2::Stream* stream)
: width(stream->width), height(stream->height), stream(stream){
