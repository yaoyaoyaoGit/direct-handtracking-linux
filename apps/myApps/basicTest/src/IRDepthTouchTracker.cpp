...
void IRDepthTouchTracker::buildEdgeImage() {
	...
	uint32_t *diffPx = (uint32_t *)diffIm[front].getPixels().getData();
	uint32_t *edgePx = (uint32_t *)edgeIm[front].getPixels().getData();
	fill_n(edgePx, n, 0);

	/* Build IR canny map */
	uint16_t *irPx = stream->getIrPixels().getData();
...
void IRDepthTouchTracker::buildDiffImage() {
	const int n = w * h;
	uint16_t *depthPx = stream->getDepthPixels().getData();
	// uint16_t *depthPx = depthStream.getPixelsRef().getPixels();
	uint32_t *diffPx = (uint32_t *)diffIm[front].getPixels().getData();

	const float *bgmean = background.getBackgroundMean().getData();
	const float *bgstdev = background.getBackgroundStdev().getData();
...
void IRDepthTouchTracker::rejectBlob(const vector<unsigned> &blob, int reason) {
	uint32_t *blobPx = (uint32_t *)blobIm[front].getPixels().getData();
...
IRDepthTouchTracker::IRDepthTouchTracker(ofxFreenect2::Stream *stream, BackgroundUpdaterThread &background)
: TouchTracker(stream, background) {
	front = 0;
